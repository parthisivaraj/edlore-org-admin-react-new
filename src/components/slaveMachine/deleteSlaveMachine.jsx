...
const DeleteSlaveMachineModal = ({
  ...
  const deleteSlaveMachine = async () => {
    try {
      setIsLoading(true);

      await nodeInstance({
        url: `slave_machines/${id}`,
        method: "DELETE",
      });
      ...
      console.error("Error deleting slave machine:", error);
    } finally {
      setIsLoading(false);
    }
  };
...
export default DeleteSlaveMachineModal;
