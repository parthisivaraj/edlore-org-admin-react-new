...
const CreateAlarmCode = ({ model_id }) => {
  ...
  const handleChangeEvent = (event) => {
    ...
    switch (name) {
      case 'title':
        errors.title = (value === "" || value.replace(/\s+/g, '').length == 0) ? "Enter Alarm Code Title" : value.length > 150 ? "Title shouldn't exceed more than 150 characters" : "";
        break;
      case "code":
        errors.code = (value === "" || value.replace(/\s+/g, '').length == 0) ? "Enter Alarm Code" : "";
        break;
      default:
        break;
    }
  ...
  const handleSubmit = (event) => {
    ...
    if (validateForm(state.errors)) {
      ...
      let errors = state.errors;
      if (state.title == "") {
        errors.title = "Enter Alarm Code Title"
      }
      if (state.code == "") {
        errors.code = "Enter Alarm Code"
      }
      setState((prevProps) => ({
        ...prevProps,
        errors: errors,
        ...
        <Dialog as="div" open={errorCodeModel} onClose={() => handleModalBackdrop(false)} className="fixed inset-0 z-50 py-10 flex items-start justify-center bg-black2 dark:bg-darkMainBg bg-opacity-40 dark:bg-opacity-60">
          ...
          >
            <Dialog.Panel className="w-[96%] lg:w-[80%] 2xl:w-[65%]  h-[90vh] relative bg-gray4 dark:bg-darkBg dark:text-gray2 border border-gray4 dark:border-opacity-20 rounded-2xl py-8  shadow-lg overflow-hidden">
              <Dialog.Title className="dark:text-gray2 text-2xl 2xl:text-3xl font-bold text-center mb-10">{update ? "Update" : "Add"} Alarm Code</Dialog.Title>
              <div className="h-[600px] px-8 overflow-scroll scrollbar-thin scrollbar-thumb-gray2 dark:scrollbar-thumb-darkMainBg scrollbar-track-gray4 dark:scrollbar-track-black1 scrollbar-thumb-rounded-3xl scrollbar-track-rounded-3xl">
                <form>
                  <div className="grid grid-cols-2 gap-4 mb-20">
                    ...
                    <div className="col-start-1 xl:col-start-2 col-span-2 xl:col-span-1">
                      <label htmlFor="code" className="text-sm font-medium dark:text-gray2">
                        <span className="whitespace-nowrap">Alarm Code</span>
                        <span className="text-danger">*</span>
                        <span className='text-gray3 text-sm ml-1'> (Please enter unique Alarm Code)</span>
                      </label> <br />
                      <input
                        type="text"
                        id="code"
                    ...
                    <div className="col-start-1 col-span-2">
                      ...
                      <textarea
                        ...
                        id="description"
                        name="description"
                        placeholder="Alarm Code Description..."
                        className="w-full text-base dark:bg-darkBg border border-gray2 dark:border-opacity-50 rounded-md py-3 px-4 mt-1 focus:border-secondary focus:outline-none"
                          ...
                          {tabs.map((tab, index) => {
                            ...
                            return (
                              <Tab
                                ...
                                className={({ selected }) =>
                                  selected ?
                                    'inline-flex items-center text-black2 dark:text-gray2 font-bold border-b-4 border-primary mr-5 xl:mr-10  focus:outline-none focus-visible:ring-0 '
                                    :
                                    'inline-flex items-center text-black2 dark:text-gray2 opacity-50 font-bold mr-5 xl:mr-10 border-none transition-all hover:opacity-100 hover:transition-all  focus:outline-none focus-visible:ring-0'
                                }
                              >
                                {title}
                              </Tab>
                  ...
                  <div className="fixed bottom-0 right-0 z-[20] bg-gray4 dark:bg-darkBg w-full flex items-center justify-between py-6 px-8 mt-8">
                    <div>
                      {update &&
                        <button
                          type="button"
                    ...
                    <div>
                      ...
                      >
                        {update ? (updateErrorCodeLoading ? "Updating..." : "Update Alarm Code") : (addErrorCodeLoading ? "Adding..." : "Add Alarm Code")}
                      </button>
                    </div>
...
export default CreateAlarmCode;